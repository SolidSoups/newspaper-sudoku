...
void assetDefaults::loadDefaultTextures() {
  ...
  props.srcSizeX = 64;
  props.srcSizeY = 640;
  AssetManager::createCachedGETexture("assets/numbers_toolbar_ui_backdrop.png",
                                      props);
...
ge::Texture *
AssetManager::createCachedGETexture(const char *filePath,
                                    const ge::TextureProperties &props) {
...
ge::Texture *AssetManager::retrieveGETextureCache(const char *filePath) {
  if (geTextureCache.find(filePath) == geTextureCache.end())
    ge::throw_runtime_error("AssetManager", "Could not find texture '" +
                                                std::string(filePath) +
                                                "' in cache!");
  return geTextureCache.at(filePath).get();
}

ge::TextureSheet *AssetManager::createCachedGETextureSheet(
    const char *filePath, const ge::TextureSplitProperties &props) {
  ge::log_msg("AssetManager", "Creating cached ge::TextureSheet '" +
                                  std::string(filePath) + "'");
...
AssetManager::retrieveGETextureSheetCache(const char *filePath) {
  if (geTextureSheetCache.find(filePath) == geTextureSheetCache.end())
    ge::throw_runtime_error("AssetManager",
                            "Could not find the texture sheet '" +
                                std::string(filePath) + "' in cache!");
