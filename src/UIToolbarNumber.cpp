...
void
UIToolbarNumber::onInit()
{
    redNumberPlateTexture = AssetManager::retrieveGETextureCache(
        "assets/red_number_plate.png");
    miniNumbersTextureSheet = AssetManager::retrieveGETextureSheetCache(
        "assets/mini-numbers.png");
}

void
UIToolbarNumber::update(const float &deltaTime)
{
...
void
UIToolbarNumber::render()
{
    drawAnimation();
}

void
UIToolbarNumber::onClean()
{
}
void
UIToolbarNumber::onDestroy()
{
}
...
UIToolbarNumber::drawAnimation()
{
    ...
    SDL_FRect plateDst = { transform.position.x - offsetAmount,
        transform.position.y - offsetAmount,
        redPlateNewSize,
        redPlateNewSize };
    ...
    ge::GameRenderer::drawTexture(
        miniNumbersTextureSheet->textures[textureID], numberDst);
}
